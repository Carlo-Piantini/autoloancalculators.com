...
function main() { 
    ...
    jQuery(document).ready(function($) { 
        ...
        $('#calc-widget-ui #calculator-widget-container').html(widget_html);

        $('#calc-widget-ui #alc-calculator .form-input').change(function() {
        ...
        $('#calc-widget-ui #calculator-widget-container .widget-submit').click(function(e) {
            ...
            if (validate_form(form_id) == true) {
                var price = sanitize_currency($('#calc-widget-ui #calculator-widget-container #alc-price').val()); // B8 in the excel sheet
                var down_pymnt = sanitize_currency($('#calc-widget-ui #calculator-widget-container #alc-down-pymnt').val()); // B9 in the excel sheet
                var interest_rate = $('#calc-widget-ui #calculator-widget-container #alc-rate').val(); // B10 in the excel sheet
                var term = $('#calc-widget-ui #calculator-widget-container #alc-term').val(); // B11 in the excel sheet
                var rebate = sanitize_currency($('#calc-widget-ui #calculator-widget-container #alc-rebate').val()); // B12 in the excel sheet
                var trade_in = sanitize_currency($('#calc-widget-ui #calculator-widget-container #alc-trade-in').val()); // B13 in the excel sheet
                var amnt_owed = sanitize_currency($('#calc-widget-ui #calculator-widget-container #alc-amnt-owed').val()); // B14 in the excel sheet
                var sales_tax = $('#calc-widget-ui #calculator-widget-container #alc-sales-tax').val(); // B15 in the excel sheet
                ...
                var results = {
                    result_fields : {
                        mnthly_pymnt : {
                            result_id : '#calc-widget-ui #calculator-widget-container #alc-mnthly-pymnt .alc-total-amnt',
                            result_value : mnthly_pymnt.toLocaleString('en-US', { style: 'currency', currency: 'USD' }),
                        },
                        loan_amnt : {
                            result_id : '#calc-widget-ui #calculator-widget-container #alc-total-loan .alc-total-amnt',
                            result_value : loan_amnt.toLocaleString('en-US', { style: 'currency', currency: 'USD' }),
                        }
                    },
                }
                update_results(results);
                $('#calc-widget-ui #calculator-widget #alc-calculator #alc-results').show();
            }
        });
        ...
        function validate_form(form_id) {
            $('#calc-widget-ui #calculator-widget-container ' + form_id + ' .form-input').removeClass('error-field');
            var valid = true;
            $('#calc-widget-ui #calculator-widget-container ' + form_id + ' .form-input').each(function() {
                if (!$(this).val()) {
                    $(this).addClass('error-field');
                    $('#calc-widget-ui #calculator-widget-container .calculator-error-msg').html("Please fill in the highlighted fields.");
                    valid = false;
                }
                if (valid) {
                    $('#calc-widget-ui #calculator-widget-container .calculator-error-msg').html('');
                }
            });
            return valid;
        }
